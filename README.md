...
| 10 | 开机自启 | `BOOT_COMPLETED` + `MY_PACKAGE_REPLACED`（需小米自启动授权） |
| 11 | 权限引导 | 主界面显示悬浮窗/电池/通知三态，一键跳转设置、小米自启动页 |
| 12 | 位置记忆 | 悬浮钮位置写入 `SharedPreferences`，重启后恢复 |
...
小米/红米在 HyperOS 3 下会激进杀后台、限制自启动，请按以下步骤保证蒙版常驻：

1. 打开本应用 → 授予「悬浮窗」权限（主界面按钮跳转）。
2. 设置 → 应用设置 → 深色蒙版 → 省电策略设为「无限制」。
3. 授权管理 / 自启动管理 → 开启「深色蒙版」自启动。
4. 多任务界面长按本应用 → 锁定（🔒），防止被划掉。
5. 完成前 3 步后，息屏/切后台再回来，蒙版仍在。

> 说明：`SYSTEM_ALERT_WINDOW`、`REQUEST_IGNORE_BATTERY_OPTIMIZATIONS`、自启动授权在
> HyperOS 上常被收纳在「手机管家 / 安全中心」中，应用内跳转不可用时会回退到应用详情页手动开启。
